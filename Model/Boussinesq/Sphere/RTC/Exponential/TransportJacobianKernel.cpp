 ...
 * @brief Source of physical space kernel for the Transport Jacobian
 */
...
std::size_t TransportJacobianKernel::name() const
{
   return this->mName;
}
   ...
   std::visit(
      [&](auto&& v, auto&& jv, auto&& t, auto&& jt)
      {
         ...
         Physical::SphericalHeatAdvection<FieldComponents::Physical::R,
            FieldComponents::Physical::THETA,
            FieldComponents::Physical::PHI>::add(rNLComp, v->dom(0).res(),
            this->mRadius, jv->dom(0).phys(), t->dom(0).grad(),
            this->mTransport);
      },
      this->vector(this->mVName), this->vector(this->mJVName), this->scalar(this->mName), this->scalar(this->mJName));
}
